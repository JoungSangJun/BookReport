...
class RegisterViewModel : ViewModel() {
    ...
    var mPassFlag : Int = 0
    var mPassConfirmFlag : Int = 0
    var mEmailFlag : Int = 0
    ...
    fun btnRegister(){
        ...
        if(edt_name.value.toString().isNotEmpty() && edt_email.value.toString().isNotEmpty()
            && edt_pass.value.toString().isNotEmpty() && edt_pass.value.toString() == edt_pass_confirm.value.toString()){

            auth.createUserWithEmailAndPassword(edt_email.value.toString(), edt_pass.value.toString()).addOnCompleteListener { task ->
                if(task.isSuccessful){

                    val firebaseUser: FirebaseUser? = auth.currentUser
                    val userUid = firebaseUser?.uid!!
                    val reference = FirebaseDatabase.getInstance().reference.child("Users").child(userUid).child("userAccount") //id 정보 담는 테이블
...
//                    postReference.setValue(postHashMap)
...
//            jBinding.emailGuidance.setTextColor(Color.parseColor("#FF0000"))
    ...
    fun checkPassword():Boolean{ //비밀번호 유효성 검사
        ...
        val passwordValidation = "^(?=.*[a-zA-Z0-9])(?=.*[a-zA-Z!@#$%^&*])(?=.*[0-9!@#$%^&*]).{6,12}$"
        ...
        if (p) {
            ...
            if(edt_pass_confirm.value.toString().isNotEmpty()) {
                mPassConfirmFlag = if (edt_pass.value.toString() == edt_pass_confirm.value.toString()) {
    ...
    fun checkPasswordConfirm():Boolean{ //비밀번호 재확인 검사
        return if (edt_pass_confirm.value.toString() == edt_pass.value.toString()) { //비밀번호가 일치할 때
            ...
            pass_confirm_vaildation.value = "* 비밀번호가 일치하지 않습니다."
            mPassConfirmFlag = 0
            //또는 passwordConfirm.setTextColor(R.color.red.toInt())
            false
        }
    }
